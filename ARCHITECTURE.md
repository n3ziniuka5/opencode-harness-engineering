...
- `src/agents/discovery.ts`: shared GPT-6 discovery prompt section used by `ask`, `brainstorm`, and `draft`.
...
- `explore` uses `openai/gpt-6-luna` with the `high` variant for cheap, high-volume source location and retrieval. It denies wildcard access plus edit, nested task, and todowrite, while allowing local read/search tools, read-only bash when needed, `webfetch`, `websearch`, `context7_*`, and ask-gated external directory reads. Recursive search defaults to the active workspace; known external repositories and dependency caches are exact target roots rather than permission to search their parent directories.
- `ask`: primary answer agent registered as `config.agent.ask` for concise, evidence-backed answers rather than implementation. It uses `openai/gpt-6-sol` with the `high` variant, `color: "accent"`, denies edits, explicitly allows common discovery and coordination tools, and allows only `task.explore` delegation while leaving unspecified tools to OpenCode's default ask behavior.
- `brainstorm`: primary ideation agent registered as `config.agent.brainstorm` for creative option generation, tradeoffs, and convergence before implementation. It uses `openai/gpt-6-sol` with the `high` variant, `color: "success"`, and the same edit-denying discovery permission posture as `ask`.
...
- `draft` uses `openai/gpt-6-sol` with the `high` variant, `color: "primary"`, and an outcome-first prompt reviewed against the OpenAI GPT-6 prompting reference. All four GPT-6 configs omit unsupported sampling parameters.
...
- `ask`, `brainstorm`, and `draft` share a static `# Discovery` section for repository context, parallel decomposition into concrete `explore` retrieval requests, durable docs, doc/code conflict handling, and explicit user-over-skill priority with transparent reporting of skill-caused pauses or redirects. The primary agents own synthesis, conflict resolution, correctness and sufficiency judgments, recommendations, and final output; the Task-visible `explore` description and runtime prompt reject general review or reasoning delegation.
...
- Check the selected provider model's parameter constraints before adding agent config fields; GPT-6 agents at non-`none` reasoning effort must omit unsupported sampling parameters.
