...
- `ask`, `brainstorm`, and `draft` prompts follow the guide's suggested structure: role, personality, goal, discovery, success criteria, constraints, output, and stop rules. `draft` also names the active plan path and archive behavior because file persistence is part of the agent contract.
