...
- The prompt includes a plan-specific `# Discovery` section for planning rules: inspect enough repository context, delegate substantial discovery to `explore`, search durable docs and local instructions, gather similar implementation patterns, apply relevant guidance to the plan, and name documentation updates when docs and implementation patterns conflict.
...
- The agent should launch parallel `explore` subagents when independent discovery questions can be answered separately.
