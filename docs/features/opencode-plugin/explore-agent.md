...
- Default project guidance (`AGENTS.md`, instructions, relevant docs, and design patterns) stays in the current Git worktree. From a nested directory, `explore` may use read-only `git rev-parse --show-toplevel` to identify the root and inspect specific ancestor guidance files only up to that root, without recursively searching an ancestor. Outside Git, it looks for default guidance only in the current directory and subtree, without ascending into parent or home directories. Within Git, repository-related patterns and docs default to the current repository.
- For a relevant related implementation or dependency, `explore` may identify a sibling repository itself, even without a caller-supplied path. For that or a caller-requested sibling or external target, it may use one shallow, non-recursive parent listing to identify the exact child, then must root recursive searches at that child. A target written as `../other-repo` is normalized to the child's exact absolute repository root before recursive search; it is not treated as a search rooted at `..`. Sibling guidance is not default guidance for the current project. If the agent cannot resolve the exact target, it reports the attempted resolution under `Missing evidence/search limitations:` instead of guessing or broadening the search.
