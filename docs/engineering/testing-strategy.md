...
- `test/plugin.test.ts` verifies the plugin id, server export, absence of bundled tools, `explore` registration, `ask` and `brainstorm` primary-agent registration, sampling, prompt, discovery, permission sentinels, `draft` registration and sampling sentinels, native `plan` disablement, `default_agent` rewrite behavior, and direct overriding of existing bundled agent entries.
