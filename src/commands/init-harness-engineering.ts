...
- **Maintain test isolation.** Each test owns setup and teardown and must not depend on ordering or shared state. When sharing testcontainers for speed, each test still needs fresh data, such as a fresh DB schema/transaction, unique topic, unique Restate key, or equivalent isolated state.
