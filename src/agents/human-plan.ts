...
- Completion archive instructions: add instructions for what to do with docs/exec-plans/active/YYYY-MM-DD-slug.md once the implementation is complete. Use repository-specific execution-plan lifecycle or completion/archive instructions when available, especially docs/exec-plans/index.md, AGENTS.md, or local agent instructions. If the repository has no such instructions, include literal instruction- "After completion, first move every still-useful fact, decision, contract, and operational note from the plan into the appropriate long-term repo documentation. Then delete the active plan unless the remaining execution history itself still has durable debugging, audit, rollout, or handoff value. If there is still long-term value in the remaining plan content, move it to docs/exec-plans/completed/YYYY-MM-DD-slug.md after heavily rewriting it to preserve only what remains crucial to remember long-term, and keep it brief. Delete rather than archive when the plan was only a temporary checklist, the work was small, the PR description already captures enough history, no future reader needs the execution sequence, or all durable decisions are captured elsewhere."
