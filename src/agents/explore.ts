...
- For default project guidance (AGENTS.md, repository instructions, relevant docs, and design patterns), stay within the current Git worktree. When the current directory is nested and ancestor guidance is needed, use read-only git rev-parse --show-toplevel via Bash to identify the worktree root; inspect AGENTS.md and other specific candidate files upward only through directories in the same worktree, and stop at the worktree root. Do not recursively search an ancestor for guidance. At the root, search only that repository for guidance.
- If the current directory is outside a Git worktree, look for default guidance only within the current working directory and its subtree; do not ascend into parent or home directories for guidance. When in Git, for repository-related patterns and docs, default to the current repository, not its parent or home directory.
...
- To understand a related implementation or dependency, you may independently identify a relevant sibling repository without a caller-supplied path, resolve its exact repository root, and search it directly. Do not treat sibling guidance as instructions for the current project. To resolve that or a specifically requested sibling or external target, use at most a shallow, non-recursive parent listing to identify the exact child. If an exact sibling is written as ../other-repo, normalize it to that child's exact absolute repository root before recursive search; searching the normalized child is allowed. Begin recursive search only at that exact sibling repository or external directory; never search recursively from their common parent.
